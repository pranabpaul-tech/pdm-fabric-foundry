...
| Area | State |
|---|---|
| Bicep waves 0-3 (network, F-SKU capacity, Key Vault, monitoring, ACR, jumpbox, vendored Foundry private standard agent, Fabric private link, Bot Service + Teams channel) | Copied from `fleet-ops-copilot`, adapted, **compiles** (`az bicep build`); **not yet deployed** for PdM |
| Fabric setup 01-04 (workspace, Eventhouse, KQL schema, downtime Eventstream) | Adapted; KQL schema is PdM-specific; **not yet run** against a live workspace |
| KQL: telemetry enrichment update policy, materialized views, ops functions, anomaly function, alert/disposition/approval tables | Written; parser + schema consistency covered by offline tests; **not yet applied** to a live Eventhouse |
...
| Operations Agent definition (3 monitoring rules) | PdM instructions written into the captured definition; portal *Generate Playbook* / *Start* still manual |
